...
void *convolute(void* arguments){
    ...
    int localStart = (srcImage->height/num_threads)*my_rank + 1;
    int localEnd = (srcImage->height/num_threads)*(my_rank+1);
    int actualEnd = min(localEnd, srcImage->height);
    span=srcImage->bpp*srcImage->bpp;
    for (row=localStart;row<actualEnd;row++){
...
int main(int argc,char** argv){
    ...
    for (long thread = 0; thread < NUM_THREADS; thread++) {
        struct thread_args_struct *args = malloc(sizeof(struct thread_args_struct));
